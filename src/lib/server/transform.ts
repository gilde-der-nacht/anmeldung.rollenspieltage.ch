	...
	return (entry) => {
		if (entry.gamemaster !== null) {
			...
			return {
				...
				gamemaster: {
					...entry.gamemaster,
					players: entry.gamemaster.players
						.slice(0, index)
						.concat(entry.gamemaster.players.slice(index + 1)),
				},
			};
		}
		if (entry.player !== null) {
			...
			return {
				...
				player: {
					...entry.player,
					players: entry.player.players
						.slice(0, index)
						.concat(entry.player.players.slice(index + 1)),
				},
			};
		}
		return entry;
...
function addPlayer(playerName: string): ChainFn {
	return (entry) => {
		if (entry.gamemaster !== null) {
			return {
				...entry,
				gamemaster: { ...entry.gamemaster, players: [...entry.gamemaster.players, playerName] },
			};
		}
		if (entry.player !== null) {
			return {
				...entry,
				player: { ...entry.player, players: [...entry.player.players, playerName] },
			};
		}
		return entry;
	};
...
function transformEntry(entry: EntryData, day: Day, hour: number, id: string): EntryData {
	...
	return chain(entry)
		...
		.change(
			pos({ ids: ['3f7bb128-fb3f-43fe-ae3c-0cf9307caf47'], days: ['sa'], hours: [] }),
			removeEverything(),
		)
		...
		.change(
			pos({ ids: ['edb358ec-eaed-4183-8352-b68fd1fde782'], days: ['sa'], hours: [14, 15] }),
			changeJobs({ kiosk: true }),
		)
